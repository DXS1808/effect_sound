...
import static com.reactlibrary.un4seen.bass.BASS.BASS_FXSetParameters;
...
import com.reactlibrary.un4seen.bass.BASSmix;
...
public class DBMediaPlayer implements IDBMediaConstants {
    ...
    public boolean prepareAudio() {
        if (!StringUtils.isEmptyString(mMediaPath)) {
            if (mMediaPath.toLowerCase(Locale.getDefault()).endsWith(TYPE_MP3)
                    ...
                    || mMediaPath.toLowerCase(Locale.getDefault()).endsWith(TYPE_AAC)
                    || mMediaPath.toLowerCase(Locale.getDefault()).endsWith(TYPE_MIDI)
                    || mMediaPath.toLowerCase(Locale.getDefault()).endsWith(TYPE_WMA)) {
                this.initMedia();
                return true;
            } else {
        ...
        if (mChanPlay != 0) {
            if (values != null) {
                ...
                if (mFxPhaserEffect != 0) {
                    ...
                    p.fRange = values[4];
                    p.fFreq = values[5];
                    boolean b = BASS_FXSetParameters(mFxPhaserEffect, p);
    ...
    public void mute() {
        ...
        BASS.BASS_ChannelSetAttribute(srcChan, BASS_FX.BASS_FX_BFX_VOLUME, 0.0f);
    }
...
//                            DBLog.d(TAG, "=========>add channel play=" + b + "===>mChanPlay=" + mTempChanPlay + "==>mChanTemp=" + mChanMix);
    ...
    public File getTempDirectory() {
        try {
            File mFileRoot = null;
            if (hasSDcard()) {
                mFileRoot = new File(Environment.getExternalStorageDirectory(), "VoiceEffects");
                if (!mFileRoot.exists()) {
                    mFileRoot.mkdirs();
                }
                File mFileCache = new File(mFileRoot, ".temp");
